# Voxel Combat Arena v9.1 Sight Guide

Version 9.1 renders a dedicated camera-mounted weapon-and-arms viewmodel and uses the weapon model itself for ordinary aiming. The center of each sight is projected to the camera center and the weapon remains visible throughout the ADS transition.
...
ADS visibility roles are cached when a viewmodel is created, so opacity transitions do not traverse and reclassify every mesh each frame.
