...
if command -v flock >/dev/null 2>&1; then
  exec 9>/var/lock/voxel-arena-update.lock
  flock -n 9 || { echo "Another Voxel Arena update is already running"; exit 1; }
fi
...
echo "=== Updating Voxel Combat Arena v9.1 ==="
...
for attempt in $(seq 1 35); do
  if curl -fsS http://127.0.0.1:3000/api/status >/tmp/voxel-status.json && grep -q '"version":"9.1.0"' /tmp/voxel-status.json; then
    cat /tmp/voxel-status.json; echo
    rm -rf "$BACKUP_DIR" /tmp/voxel-npm-cache
    SWAPPED=0
    trap - ERR
    echo "=== Voxel Combat Arena v9.1 update complete ==="
    exit 0
  fi
  sleep 2
done
