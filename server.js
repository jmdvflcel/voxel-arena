...
function simulateMovement(player, dt, now) {
  ...
  if (player.grapple) {
    if (now >= player.grapple.endsAt) {
      ...
      player.grapple.ropeLength = Math.max(
        GRAPPLE_STOP_DISTANCE,
        player.grapple.ropeLength - GRAPPLE_REEL_SPEED * dt * (player.input.forward ? 1.3 : 1)
      );
...
function handleMelee(attacker, message) {
  ...
  attacker.nextAttackAt = now + weapon.cooldown + attacker.combo * 36;
  ...
  for (const candidate of candidates.slice(0, maxHits)) {
    const result = applyDamage(attacker, candidate.target, weapon.damages[combo] * (confirmedHits === 0 ? 1 : 0.82), {
      kind: "melee",
      weapon: weaponName,
...
function handleGrapple(player, message = {}) {
  ...
  if (!hit) {
    player.grappleReadyAt = now + 260;
    send(player.ws, { type: "grapple_miss", readyAt: player.grappleReadyAt });
    return;
  }

  const ropeLength = clamp(hit.distance - 3.2, GRAPPLE_STOP_DISTANCE, GRAPPLE_RANGE);
  player.grapple = {
    target: hit.target,
    endsAt: now + GRAPPLE_DURATION_MS,
    attachedAt: now,
    ropeLength
  };
  player.grappleReadyAt = now + GRAPPLE_COOLDOWN_MS;
  recordAction(player, { type: "grapple", target: hit.target });
  broadcast({
    type: "grapple_start",
    id: player.id,
    origin,
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`Voxel Combat Arena v7 listening on port ${PORT}`);
