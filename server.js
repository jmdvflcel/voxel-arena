...
function humanPlayerCount() {
  let count = 0;
  for (const client of clients.values()) if (!client.player.isBot) count++;
  return count;
}

function botPlayerCount() {
  let count = 0;
  for (const client of clients.values()) if (client.player.isBot) count++;
  return count;
}
...
function effectiveSpread(player, weapon, aiming, now) {
  ...
  const movementScale = aiming ? 0.42 : 1;
  spread += Math.min(1, speed / Math.max(SPRINT_SPEED, 1)) * weapon.moveSpread * movementScale;
  if (!grounded) spread += weapon.airSpread * (aiming ? 0.72 : 1);
  if (player.input.crouch && grounded) spread *= 0.62;
  if (player.input.steady && aiming && grounded && speed < 1.2 && player.stamina > 0) spread *= 0.42;
  if (aiming && grounded && speed < 1.15 && now - player.lastShotAt > 300) spread *= 0.2;
  // Visible camera recoil is now the primary sustained-fire penalty in ADS.
  spread += player.weaponBloom * (aiming ? 0.28 : 1);
...
function nearestEnemyFor(player) {
  let best = null;
  let bestDistance = Infinity;
  for (const client of clients.values()) {
    const other = client.player;
    if (!validTarget(player, other, Date.now())) continue;
...
function updateBots(now) {
  for (const client of clients.values()) {
    const bot = client.player;
    if (!bot.isBot || !bot.alive || round.status !== "playing") continue;
    const targetInfo = nearestEnemyFor(bot);
    if (!targetInfo) { bot.input.forward = false; continue; }
    const target = targetInfo.player;
    const dx = target.x - bot.x;
    const dz = target.z - bot.z;
    bot.yaw = Math.atan2(-dx, -dz);
    bot.pitch = clamp(Math.atan2((target.y + 1.1) - (bot.y + 1.45), Math.hypot(dx, dz)), -0.8, 0.8);
    bot.input.forward = targetInfo.distance > 8;
    bot.input.backward = targetInfo.distance < 3;
    bot.input.left = Math.sin(now * 0.001 + bot.botPhase) > 0.72;
    bot.input.right = Math.sin(now * 0.001 + bot.botPhase) < -0.72;
    bot.input.sprint = targetInfo.distance > 15;
    bot.input.crouch = false;
    bot.input.aiming = targetInfo.distance > 9;
    bot.input.steady = targetInfo.distance > 18;
    if (now >= bot.botNextActionAt) {
      if (targetInfo.distance < 3.2) { bot.weapon = "sword"; handleMelee(bot, { attackType: Math.random() < 0.22 ? "heavy" : "light", latency: 0 }); }
      else {
        bot.weapon = targetInfo.distance > 34 ? "marksman" : "rifle";
        bot.owned.add(bot.weapon);
        const magazine = WEAPONS[bot.weapon]?.magazine || 30;
        if (!bot.ammo[bot.weapon] || bot.ammo[bot.weapon].magazine <= 0) bot.ammo[bot.weapon] = { magazine, reserve: magazine * 4 };
        handleHitscan(bot, bot.weapon, { yaw: bot.yaw, pitch: bot.pitch, aiming: bot.input.aiming, latency: 0 });
      }
      bot.botNextActionAt = now + 120 + Math.random() * 280;
    }
  }
}
...
function createBot(index) {
  const id = `bot-${index}-${crypto.randomUUID().slice(0, 4)}`;
  const team = chooseTeam();
  const spawn = chooseSafeSpawn(team, id);
  const player = {
    id, ws: null, name: ["Axiom", "Nova", "Rook", "Viper", "Echo", "Atlas", "Cipher", "Mako"][index % 8], team, color: teamColor(team), ...spawn,
    vx: 0, vy: 0, vz: 0, yaw: spawn.yaw, pitch: 0, health: 100, armor: 25,
    kills: 0, deaths: 0, assists: 0, roundScore: 0, damageDealt: 0, shots: 0, hits: 0, streak: 0, bestStreak: 0, captures: 0,
    stamina: STAMINA_MAX, skin: ["carbon", "neon", "standard"][index % 3], archetype: ["assault", "scout", "heavy"][index % 3], isBot: true, alive: true, respawnAt: 0,
    invulnerableUntil: Date.now() + SPAWN_PROTECTION_MS, stunnedUntil: 0, weapon: "rifle",
    owned: new Set(["sword", "pistol", "smg", "rifle", "marksman"]), ammo: makeAmmo(), reload: null, nextAttackAt: 0,
    combo: 0, lastComboAt: 0, blocking: false, blockStartedAt: 0, slideUntil: 0, dashUntil: 0, dashReadyAt: 0,
    dashDirection: { x: 0, z: 0 }, grapple: null, grappleProjectile: null, grappleReadyAt: 0, embeddedTicks: 0,
    weaponBloom: 0, lastShotAt: 0, powers: makePowers(), lastDamagedAt: 0, lastGroundedAt: Date.now(), jumpRequestedAt: -Infinity,
    lastInputSeq: 0, previousJump: false, previousCrouch: false,
    input: { forward: false, backward: false, left: false, right: false, jump: false, sprint: false, crouch: false, aiming: false, steady: false },
    history: [{ t: Date.now(), x: spawn.x, y: spawn.y, z: spawn.z, yaw: spawn.yaw, pitch: 0, weapon: "rifle", crouching: false }],
    actionHistory: [], damageContributors: new Map(), botPhase: Math.random() * Math.PI * 2, botNextActionAt: Date.now() + 1000
  };
  clients.set(id, { ws: null, player, lastMessageAt: Date.now(), messageWindowAt: Date.now(), messageCount: 0 });
  broadcast({ type: "system", text: `${player.name} [BOT] joined the arena` });
}
