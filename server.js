...
function effectiveSpread(player, weapon, aiming, now) {
  ...
  if (player.input.steady && aiming && grounded && speed < 1.2 && player.stamina > 0) spread *= 0.48;
  if (aiming && grounded && speed < 1.15 && now - player.lastShotAt > 300) spread *= 0.32;
...
function handleHitscan(attacker, weaponName, message) {
  ...
  const baseDirection = directionFromAngles(shotYaw, shotPitch);
