...
const configuredMaxClients = Number(process.env.MAX_CLIENTS || 32);
const MAX_CLIENTS = Number.isFinite(configuredMaxClients) ? Math.max(1, Math.min(64, Math.floor(configuredMaxClients))) : 32;
...
const MAX_SNAPSHOT_BUFFER = 384 * 1024;
const MAX_CONNECTION_BUFFER = 2 * 1024 * 1024;
...
function send(ws, payload) {
  ...
  if (ws.bufferedAmount > MAX_CONNECTION_BUFFER) {
    ws.terminate();
    return false;
  }
  try {
...
function broadcast(payload, exceptId = null) {
  ...
  for (const [id, client] of clients) {
    const ws = client.ws;
    if (id === exceptId || !ws || ws.readyState !== WebSocket.OPEN) continue;
    if (ws.bufferedAmount > MAX_CONNECTION_BUFFER) {
      ws.terminate();
      continue;
    }
...
function effectiveSpread(player, weapon, aiming, now, adsAmount = 0) {
  ...
  const fullyAimed = aiming && adsAmount >= 0.82;
  // Single-projectile firearms are sight-true at full ADS. Recoil moves the
  // camera/reticle itself, so additional random spread would contradict it.
  if (fullyAimed && Number(weapon.pellets || 1) === 1) return 0;
...
function handleHitscan(attacker, weaponName, message) {
  ...
  const adsAmount = clamp(Number(message.adsAmount) || 0, 0, 1);
  const shotSpread = effectiveSpread(attacker, weapon, aiming, now, adsAmount);
  attacker.weaponBloom = Math.min(weapon.maxBloom, attacker.weaponBloom + weapon.bloomPerShot);
  attacker.lastShotAt = now;
  const defaultEyeHeight = attacker.input.crouch ? 1.18 : 1.62;
  const origin = { x: attacker.x, y: attacker.y + defaultEyeHeight, z: attacker.z };
  const requestedOrigin = message.origin;
  if (requestedOrigin && Number.isFinite(requestedOrigin.x) && Number.isFinite(requestedOrigin.y) && Number.isFinite(requestedOrigin.z)) {
    const horizontalError = Math.hypot(requestedOrigin.x - attacker.x, requestedOrigin.z - attacker.z);
    const minimumEye = attacker.y + (attacker.input.crouch ? 1.02 : 1.42);
    const maximumEye = attacker.y + (attacker.input.crouch ? 1.36 : 1.76);
    if (horizontalError <= 0.28 && requestedOrigin.y >= minimumEye && requestedOrigin.y <= maximumEye) {
      origin.x = requestedOrigin.x;
      origin.y = requestedOrigin.y;
      origin.z = requestedOrigin.z;
    }
  }
...
function snapshotFor(player, publicStates, roster, roundSnapshot, serverNow) {
  const visiblePlayers = [];
  for (const target of publicStates) {
    const distance = Math.hypot(target.x - player.x, target.z - player.z);
    if (target.id === player.id || distance <= INTEREST_RADIUS) visiblePlayers.push(target);
  }
  return {
    type: "snapshot",
...
function runTick() {
  ...
  const tickSpacing = tickStartedAt - lastTickStartedAt;
  lastTickStartedAt = tickStartedAt;
  const lagSample = Math.max(0, tickSpacing - TICK_MS);
  eventLoopLagMs += (lagSample - eventLoopLagMs) * 0.08;
  ...
  if (tickNumber % SNAPSHOT_EVERY === 0 && clients.size) {
    const publicStates = Array.from(clients.values(), (client) => publicPlayer(client.player, now));
    const roster = publicStates.map(publicRosterPlayer);
    const roundSnapshot = roundPublic();
    for (const client of clients.values()) {
      const ws = client.ws;
      if (!ws || ws.readyState !== WebSocket.OPEN) continue;
      if (ws.bufferedAmount > MAX_SNAPSHOT_BUFFER) {
        droppedSnapshots++;
        continue;
      }
      send(ws, snapshotFor(client.player, publicStates, roster, roundSnapshot, now));
    }
  }
}
...
function shutdown(signal) {
  ...
  for (const client of clients.values()) {
    try { client.ws?.close(1001, "server restart"); } catch {}
  }
  wss.close();
