# Voxel Combat Arena v9.1

A human-only, server-authoritative browser arena shooter built with Node.js, WebSockets, and Three.js. Version 9.1 is a performance and operational-stability pass built on the center-clear ADS, exact aim alignment, no-bot matchmaking, and authoritative roster systems from v9.0 and v8.8.

## What v9.1 adds

### Client smoothness

- Allocation-free hot paths for camera correction, third-person camera movement, pickup/player label fading, spectator tracking, grapple ropes, and reconciliation.
- Cached DOM references and change-aware HUD writes.
- Radar, match clock, power HUD, stamina HUD, day/night lighting, and shadow maps update at purpose-appropriate rates instead of all running at display refresh rate.
- Cached ADS mesh-role lists remove repeated full viewmodel traversals.
- Allocation-free gamepad polling, shared synthesized-noise buffers, and deduplicated asset loading.
- Dynamic-resolution hysteresis avoids rapid resolution changes after brief frame-time spikes.

### Server stability

- Snapshot backpressure protection prevents slow clients from accumulating an unbounded send queue.
- Public player state, roster data, and round state are generated once per snapshot tick and reused.
- WebSocket payload limits, slow-connection termination, a configurable player cap, graceful SIGTERM/SIGINT shutdown, and event-loop lag telemetry.
- Code files use conditional revalidation to prevent stale browser JavaScript after an update, while media assets retain short caching.

### Deployment and validation

- Exact dependency versions without an environment-specific lockfile.
- Concurrent update/install locks, bounded command timeouts, version-aware health checks, and expanded systemd/nginx safeguards.
- Static, presentation, ten-client smoke, and 24-client load tests.
...
- Mode voting, map-variant voting, round highlights, scoreboard statistics, spectator camera, kill cam, private room code support, with human-only matches.
...
Bots are disabled permanently in v9.1. Set `ROOM_CODE` to require a private-match code.
