# Voxel Combat Arena v9.0

A human-only server-authoritative browser arena shooter built with Node.js, WebSockets, and Three.js. Version 8.6 preserves the v8.5 ADS and aim-truth work while fixing duplicate scoreboard rows, frozen ghost characters, and reconnect-state accumulation.

## What v9.0 adds

- An authoritative lightweight roster sent with every snapshot so scoreboard entries represent current server players only.
- Complete player-cache and remote-model cleanup on reconnection or session reinitialization.
- Removal of interest-culling ghost models without removing active players from the scoreboard.
- WebSocket generation guards that ignore stale close/message events from obsolete connections.
...
Bots are disabled permanently in v9.0. Set `ROOM_CODE` to require a private-match code.
