# Voxel Combat Arena v8.4

A server-authoritative browser arena shooter built with Node.js, WebSockets, and Three.js. Version 8.4 fixes the first-person rendering path and keeps the weapon, hands, and weapon-specific sight visible in hip-fire and ADS.

## What v8.4 adds

### First-person viewmodel reliability

- Added the game camera to the Three.js scene so camera-mounted weapon and arm meshes are actually rendered.
- Added a named `ViewModelRoot` beneath the camera and kept the equipped weapon visible during hip-fire and ordinary ADS.
- Reduced the camera near plane to 0.015 and disabled frustum culling, fog, depth testing, and depth writing for viewmodel meshes.
- Added unique hip and ADS depths, offsets, arm spacing, and transitions for every firearm.
- Restricted fullscreen scope overlays to the Longshot marksman rifle and Apex Railgun.

### Weapon-mounted aiming

- Keeps the firearm and first-person hands visible throughout the ADS transition, so the player never appears to fire from an empty camera.
- Aligns every sight anchor mathematically with the camera center rather than moving all weapons to one generic ADS position.
- Gives the pistol a squared rear notch and illuminated front post.
- Gives the SMG a compact rear aperture and front orb/post.
- Gives the shotgun a receiver rib, open rear reference, and bright front bead.
- Gives the rifle a cyan holographic ring-and-dot sight.
- Gives the LMG a protected amber-chevron reflex sight.
- Gives the burst rifle a distinct enclosed prism optic with an etched cross reticle.
- Reserves the circular screen-space scope for the marksman rifle and gives the railgun a separate rectangular digital targeting display.
- Reduces zoom on iron and reflex sights while retaining variable magnification only for weapons designed for it.
- Suppresses camera bob and sway while aligned without removing recoil feedback.
