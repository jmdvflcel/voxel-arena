...
- Removed recurring render-loop allocations from camera correction, third-person camera movement, pickup labels, remote labels, spectator tracking, grapple ropes, and reconciliation.
- Cached DOM lookups and throttled non-critical HUD, radar, day/night, and shadow-map updates while preserving smooth camera, weapon, and crosshair motion.
- Cached first-person mesh role groups so ADS visibility no longer traverses every weapon and arm mesh every frame.
- Reused gamepad state, shared synthesized-noise audio buffers, and deduplicated concurrent asset-manifest loading.
- Added WebSocket payload limits, server-side snapshot backpressure, slow-connection protection, a 32-player default cap, graceful shutdown, and event-loop health telemetry.
- Cached roster, round, and public-player state once per snapshot tick instead of rebuilding them separately for every client.
- Added deployment locks, bounded install/test timeouts, exact dependency versions, stronger systemd/nginx defaults, and version-aware health checks.
- Added a 24-client load test; validation completed with no snapshot drops and low event-loop latency.

## 9.0.0

### Center-clear ADS

- Reclassified first-person meshes into reticle, glass, housing, and body roles so oversized sight ribs and housings no longer remain fully opaque while aiming.
- Added a minimal weapon-specific center reticle for pistol, SMG, shotgun, rifle, burst rifle, and LMG ADS.
- Reduced receiver, sight-housing, glass, and arm opacity independently for every firearm.
- Moved large viewmodels farther from the camera and lowered them to preserve target visibility.
- Preserved camera-origin, camera-direction, and server-validated shot alignment.
