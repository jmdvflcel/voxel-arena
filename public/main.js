...
const SCOPE_OVERLAY_TYPES = new Set(["optic", "sniper", "rail"]);
...
function setupNetworkHandlers() {
  ...
  network.on("init", (message) => {
    ...
    $("versionText").textContent = `v${message.version || "8.2.0"}`;
...
function updateCrosshair() {
  ...
  let opacity = local.blocking ? 0.45 : 1 - adsAmount * 0.88;
  if (aiming && info.scope === "iron") opacity = 0;
  else if (aiming && info.scope === "reflex") opacity = 0.72;
  else if (aiming) opacity = 0.06;
...
function updateCameraFov(dt) {
  ...
  const adsActive = adsAmount > 0.36 && settings.cameraMode === "first" && scopeType !== "none";
  const showScope = adsActive && SCOPE_OVERLAY_TYPES.has(scopeType);
