...
function setupNetworkHandlers() {
  ...
  network.on("init", (message) => {
    ...
    $("versionText").textContent = `v${message.version || "9.0.0"}`;
...
function updateCameraFov(dt) {
  ...
  adsReticle.classList.toggle("hidden", reticleBlend <= 0.001);
  adsReticle.dataset.sight = sightStyle;
  adsReticle.style.setProperty("--reticle-ads", String(reticleBlend));
