...
function fireGun() {
  ...
  localShotId++;
  const shotYaw = local.yaw + recoilYaw + aimPunchYaw;
  const shotPitch = THREE.MathUtils.clamp(local.pitch + recoilPitch + aimPunchPitch, -1.45, 1.45);
...
function setupNetworkHandlers() {
  ...
  network.on("init", (message) => {
    ...
    $("versionText").textContent = `v${message.version || "8.5.0"}`;
