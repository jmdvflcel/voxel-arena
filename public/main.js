...
const thirdPersonViewDirection = new THREE.Vector3();
const thirdPersonRight = new THREE.Vector3();
const thirdPersonPivot = new THREE.Vector3();
const thirdPersonDesired = new THREE.Vector3();
const thirdPersonRayDirection = new THREE.Vector3();
const correctionStep = new THREE.Vector3();
const obstructionDirection = new THREE.Vector3();
const obstructionOrigin = new THREE.Vector3();
const shotAimOrigin = new THREE.Vector3();
const shotAimDirection = new THREE.Vector3();
const casingDirection = new THREE.Vector3();
const remoteCameraPosition = new THREE.Vector3();
const remoteCameraForward = new THREE.Vector3();
const remoteLabelPosition = new THREE.Vector3();
const remoteLabelDirection = new THREE.Vector3();
const spectatorBack = new THREE.Vector3();
const spectatorDesired = new THREE.Vector3();
const reconciliationError = new THREE.Vector3();
...
let lastStaminaHudSignature = "";
let lastWeaponHudSignature = "";
let lastHealthHudSignature = "";
let lastScoreboardSignature = "";
let lastRoundHudSignature = "";
let localOwnedSignature = "";
let lastCrosshairGap = Number.NaN;
let lastCrosshairOpacity = Number.NaN;
let lastScopeClassSignature = "";
let lastScopeTelemetrySignature = "";
let lastScopeBlend = -1;
let lastReticleBlend = -1;
let nextFastHudAt = 0;
let nextSlowHudAt = 0;
let nextRadarAt = 0;
let nextDayNightAt = 0;
let clientBloom = 0;
...
function updateStaminaHud() {
  ...
  const rounded = Math.round(stamina);
  const signature = `${rounded}:${stamina < 15}`;
  if (signature === lastStaminaHudSignature) return;
  lastStaminaHudSignature = signature;
  $("staminaBar").style.width = `${rounded}%`;
  $("staminaText").textContent = String(rounded);
...
function updateSpectatorCamera() {
  ...
  spectatorBack.set(Math.sin(target.yaw) * 5.5, 2.6, Math.cos(target.yaw) * 5.5);
  spectatorDesired.set(target.x, target.y + 1.3, target.z).add(spectatorBack);
  camera.position.lerp(spectatorDesired, 0.16);
...
function updateWeaponObstruction() {
  ...
  camera.getWorldDirection(obstructionDirection);
  obstructionOrigin.copy(camera.position);
  grappleRaycaster.set(obstructionOrigin, obstructionDirection);
...
function updateWeaponHud() {
  ...
  const ownedSignature = Array.from(local.owned).sort().join(",");
  const signature = `${local.weapon}:${ammo.magazine ?? 0}:${ammo.reserve ?? 0}:${local.reloading}:${local.blocking}:${local.alive}:${ownedSignature}`;
  if (signature === lastWeaponHudSignature) return;
  lastWeaponHudSignature = signature;
  ...
  $("weaponState").textContent = local.reloading ? "RELOADING" : local.blocking ? "BLOCKING" : local.alive ? "READY" : "DISABLED";
...
function setHealthArmor(health, armor) {
  ...
  const roundedHealth = Math.round(local.health);
  const roundedArmor = Math.round(local.armor);
  const healthBand = local.health > 55 ? "healthy" : local.health > 25 ? "warning" : "critical";
  const signature = `${roundedHealth}:${roundedArmor}:${healthBand}`;
  if (signature === lastHealthHudSignature) return;
  lastHealthHudSignature = signature;

  $("healthText").textContent = String(roundedHealth);
  $("armorText").textContent = String(roundedArmor);
  $("healthBar").style.width = `${roundedHealth}%`;
  $("armorBar").style.width = `${Math.min(100, roundedArmor)}%`;
  $("armorText").classList.toggle("overshield", local.armor > 100);
  $("healthBar").style.background = healthBand === "healthy"
    ? "linear-gradient(90deg,#27ad58,#69e88d)"
    : healthBand === "warning"
      ? "linear-gradient(90deg,#cb8a2b,#f0c84a)"
      : "linear-gradient(90deg,#a91f35,#ef5665)";
}
...
function updateGrappleLines() {
  ...
  for (const [id, line] of grappleLines) {
    ...
    if (id === local.id) {
      if (settings.cameraMode === "first") firstPersonWeapon.muzzleWorldPosition(start);
      else start.set(local.x, local.y + 1.35, local.z);
    } else {
      const remote = remotePlayers.get(id);
      if (remote) start.copy(remote.group.position).y += 1.35;
      else start.set(0, 0, 0);
    }
...
function updateScoreboard(force = false) {
  ...
  const signature = players
    .map((player) => `${player.id}:${player.name}:${player.team}:${player.kills || 0}:${player.deaths || 0}:${player.assists || 0}:${player.alive !== false}`)
    .sort()
    .join("|");
  if (!force && signature === lastScoreboardSignature) return;
...
function syncRoster(roster = []) {
  ...
  for (const player of roster) {
    ...
    const existing = rosterData.get(player.id) || {};
    Object.assign(existing, player);
    rosterData.set(player.id, existing);
  }
...
function applyPlayerData(player, sampleTime = performance.now()) {
  const playerRecord = playerData.get(player.id) || {};
  Object.assign(playerRecord, player);
  playerData.set(player.id, playerRecord);
  const rosterRecord = rosterData.get(player.id) || {};
  Object.assign(rosterRecord, {
    id: player.id,
    name: player.name,
    team: player.team,
    kills: player.kills,
  ...
  rosterData.set(player.id, rosterRecord);
  ...
  if (player.id === local.id) {
    ...
    if (Array.isArray(player.owned)) {
      const nextOwnedSignature = player.owned.slice().sort().join(",");
      if (nextOwnedSignature !== localOwnedSignature) {
        localOwnedSignature = nextOwnedSignature;
        local.owned = new Set(player.owned);
      }
    }
    if (player.powers) Object.assign(local.powers, player.powers);
    local.grappleReadyAt = Number(player.powers?.grappleReadyAt || local.grappleReadyAt || 0);
    if (player.grapple) local.grapple = Object.assign(local.grapple || {}, player.grapple);
...
function setRound(round) {
  ...
  const roundHudSignature = [
    round.status, round.mode, round.map, round.winner,
    Math.floor(round.score?.red || 0), Math.floor(round.score?.blue || 0),
    round.modeVotes?.tdm || 0, round.modeVotes?.koth || 0, round.modeVotes?.ffa || 0,
    round.mapVotes?.foundry || 0, round.mapVotes?.nightfall || 0, round.mapVotes?.storm || 0,
    objective.id, objective.owner, objective.capturing, Math.round(objective.progress || 0),
    highlights.eliminator?.name, highlights.eliminator?.value,
    highlights.damage?.name, highlights.damage?.value,
    highlights.objective?.name, highlights.objective?.value
  ].join(":");
  if (roundHudSignature === lastRoundHudSignature) return;
  lastRoundHudSignature = roundHudSignature;
...
function simulateLocalMovement(dt) {
  ...
  if (local.correction.lengthSq() > 0.000001) {
    ...
    correctionStep.copy(local.correction).multiplyScalar(factor);
    local.x += correctionStep.x;
    local.y += correctionStep.y;
    local.z += correctionStep.z;
    local.correction.sub(correctionStep);
  }
...
function updateCameraTransform(dt, eyeHeight, bobX, bobY) {
  ...
  thirdPersonViewDirection.set(
    -Math.sin(local.yaw) * cosPitch,
    Math.sin(pitch),
    -Math.cos(local.yaw) * cosPitch
  ).normalize();
  thirdPersonRight.set(Math.cos(local.yaw), 0, -Math.sin(local.yaw));
  thirdPersonPivot.set(
    local.x,
    local.y + Math.max(1.25, eyeHeight * 0.88),
    local.z
  );

  thirdPersonDesired.copy(thirdPersonPivot)
    .addScaledVector(thirdPersonViewDirection, -5.2)
    .addScaledVector(thirdPersonRight, 0.72);
  thirdPersonDesired.y += 0.42;

  thirdPersonRayDirection.copy(thirdPersonDesired).sub(thirdPersonPivot);
  const desiredDistance = thirdPersonRayDirection.length();
  thirdPersonRayDirection.normalize();
  thirdPersonRaycaster.set(thirdPersonPivot, thirdPersonRayDirection);
  ...
  if (hit && hit.distance < desiredDistance) {
    thirdPersonDesired.copy(thirdPersonPivot).addScaledVector(thirdPersonRayDirection, Math.max(0.55, hit.distance - 0.28));
  }

  if (thirdPersonCameraPosition.lengthSq() === 0) {
    thirdPersonCameraPosition.copy(thirdPersonDesired);
  }

  thirdPersonCameraPosition.lerp(thirdPersonDesired, 1 - Math.exp(-dt * 14));
...
function fireGun() {
  ...
  const shotYaw = Math.atan2(-shotAimDirection.x, -shotAimDirection.z);
  const shotPitch = THREE.MathUtils.clamp(Math.asin(THREE.MathUtils.clamp(shotAimDirection.y, -1, 1)), -1.45, 1.45);
  ...
  network.send("fire", {
    ...
    yaw: shotYaw,
    pitch: shotPitch,
    origin: { x: shotAimOrigin.x, y: shotAimOrigin.y, z: shotAimOrigin.z },
    direction: { x: shotAimDirection.x, y: shotAimDirection.y, z: shotAimDirection.z }
  });
  ...
  casingDirection.set(1, 0.35, 0).applyQuaternion(camera.quaternion);
  effects.spawnCasing(origin, casingDirection, local.weapon);
}
...
function setupNetworkHandlers() {
  ...
  network.on("init", (message) => {
    ...
    $("versionText").textContent = `v${message.version || "9.1.0"}`;
...
function updateGraphicsScaling(now) {
  ...
  if (fpsValue < 42) {
    lowFpsSeconds++;
    highFpsSeconds = 0;
    if (lowFpsSeconds >= 2 && now - lastResolutionChangeAt >= 1800) {
      resolutionScale = Math.max(0.58, resolutionScale - 0.07);
      lowFpsSeconds = 0;
    ...
    highFpsSeconds++;
    lowFpsSeconds = 0;
    if (highFpsSeconds >= 4 && now - lastResolutionChangeAt >= 2500) {
      resolutionScale = Math.min(1, resolutionScale + 0.04);
      highFpsSeconds = 0;
      lastResolutionChangeAt = now;
      updateRendererResolution();
    }
...
function updateCrosshair() {
  ...
  if (!Number.isFinite(lastCrosshairGap) || Math.abs(gap - lastCrosshairGap) > 0.08) {
    lastCrosshairGap = gap;
    crosshair.style.setProperty("--gap", `${gap.toFixed(2)}px`);
  }
  if (!Number.isFinite(lastCrosshairOpacity) || Math.abs(opacity - lastCrosshairOpacity) > 0.015) {
    lastCrosshairOpacity = opacity;
    crosshair.style.opacity = opacity.toFixed(3);
  }
}
...
function updateCameraFov(dt) {
  ...
  if (Math.abs(targetFov - camera.fov) < 0.002) camera.fov = targetFov;
  ...
  const scopeBlend = fullscreenScope ? THREE.MathUtils.smoothstep(adsAmount, 0.18, 0.82) : 0;
  const showScope = scopeBlend > 0.001;
  const cleanWeaponSight = adsActive && !FULLSCREEN_SCOPE_WEAPONS.has(local.weapon);
  const reticleBlend = cleanWeaponSight ? THREE.MathUtils.smoothstep(adsAmount, 0.32, 0.82) : 0;
  const scopeActive = scopeBlend > 0.2;
  const reticleVisible = reticleBlend > 0.001;
  const scopeClassSignature = `${scopeType}:${sightStyle}:${adsActive}:${showScope}:${scopeActive}:${cleanWeaponSight}:${reticleVisible}`;
  if (scopeClassSignature !== lastScopeClassSignature) {
    lastScopeClassSignature = scopeClassSignature;
    scope.classList.toggle("hidden", !showScope);
    scope.classList.toggle("active", scopeActive);
    ...
    const adsReticle = $("adsReticle");
    adsReticle.classList.toggle("hidden", !reticleVisible);
    adsReticle.dataset.sight = sightStyle;
  }
  if (Math.abs(scopeBlend - lastScopeBlend) > 0.002) {
    lastScopeBlend = scopeBlend;
    scope.style.setProperty("--ads", scopeBlend.toFixed(4));
    scope.style.setProperty("--scope-blend", scopeBlend.toFixed(4));
  }
  if (Math.abs(reticleBlend - lastReticleBlend) > 0.002) {
    lastReticleBlend = reticleBlend;
    $("adsReticle").style.setProperty("--reticle-ads", reticleBlend.toFixed(4));
  }
  ...
  const telemetryState = steady ? "BREATH HELD" : local.input.crouch && local.grounded ? "BRACED" : "TRACKING";
  const scopeTelemetrySignature = `${local.weapon}:${zoom.toFixed(1)}:${telemetryState}:${estimatedRange}`;
  if (scopeTelemetrySignature !== lastScopeTelemetrySignature) {
    lastScopeTelemetrySignature = scopeTelemetrySignature;
    $("scopeLabel").textContent = `${zoom.toFixed(1)}× · ${info.name}`;
    $("scopeTelemetry").textContent = `${telemetryState} · ${String(estimatedRange).padStart(3, "0")} m`;
  }
}

function updateRemotePlayers(now, dt) {
  camera.getWorldPosition(remoteCameraPosition);
  camera.getWorldDirection(remoteCameraForward);
  const epochNow = Date.now();
  for (const remote of remotePlayers.values()) {
    remote.interpolationDelay = THREE.MathUtils.clamp(95 + network.jitter * 1.8 + network.loss * 180, 90, 210);
    remote.render(now, dt, remote.interpolationDelay, epochNow);
    if (remote.label?.material) {
      remote.label.getWorldPosition(remoteLabelPosition);
      remoteLabelDirection.copy(remoteLabelPosition).sub(remoteCameraPosition);
      const distance = Math.max(0.001, remoteLabelDirection.length());
      const centerDot = remoteCameraForward.dot(remoteLabelDirection.multiplyScalar(1 / distance));
...
function updateLocalAvatar(now, dt) {
  ...
  local.sliding = performance.now() < local.slideUntil;
  local.crouching = local.input.crouch;
  localAvatar.renderLocal(local, now, dt, settings.cameraMode === "third");
}
...
function animate() {
  ...
  updateCrosshair();
  if (now >= nextFastHudAt) {
    nextFastHudAt = now + 50;
    updateStaminaHud();
    updateGrappleHud();
  }
  if (now >= nextSlowHudAt) {
    nextSlowHudAt = now + 250;
    updateMatchClock();
    updatePowerHud();
  }
  if (now >= nextRadarAt) {
  ...
  if (now >= nextDayNightAt) {
    nextDayNightAt = now + 100;
    updateDayNight(now);
  }
  ...
  if (quality.shadows && now >= nextShadowUpdateAt) {
    nextShadowUpdateAt = now + 1000 / Math.max(1, Number(quality.shadowFps || 30));
