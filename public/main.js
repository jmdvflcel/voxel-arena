...
function attackSword() {
  ...
  const swordDirection = {
    x: -Math.sin(local.yaw),
    y: Math.sin(local.pitch) * 0.22,
    z: -Math.cos(local.yaw)
  };
  effects.spawnSlash(local, swordDirection, info.color, localCombo);

  const lungeStrength = [4.2, 4.8, 6.1][localCombo] || 4.2;
  local.vx += -Math.sin(local.yaw) * lungeStrength;
  local.vz += -Math.cos(local.yaw) * lungeStrength;
  network.send("melee", { combo: localCombo, weapon: local.weapon, latency: network.latency });
}
...
function fireGun() {
  ...
  recoilShotIndex++;
  lastLocalShotAt = now;
  const adsControl = rightMouseDown ? 0.72 : 1;
...
function setupNetworkHandlers() {
  ...
  network.on("melee_result", (message) => {
    if (message.hits > 0) audio.playMeleeHit(message.combo === 2 || message.weapon === "voidblade");
...
function updateCameraFov(dt) {
  ...
  const adsResponse = info.scope === "sniper" || info.scope === "rail" ? 10 : 15;
  ...
  const targetFov = THREE.MathUtils.lerp(hipFov, info.fov, easedAds);
  ...
  $("scopeTelemetry").textContent = `${local.input.crouch && local.grounded ? "BRACED" : "TRACKING"} · ${String(estimatedRange).padStart(3, "0")} m`;
}
