...
Promise.all([
  assetPipeline.weapon("rifle"),
  assetPipeline.weapon("sword")
]).then(([rifle, sword]) => {
  if (rifle) firstPersonWeapon.installAuthoredModel("rifle", rifle);
  if (sword) firstPersonWeapon.installAuthoredModel("sword", sword);
}).catch(() => null);
...
function updateCameraTransform(dt, eyeHeight, bobX, bobY) {
  ...
  camera.rotation.y = local.yaw + recoilYaw * 0.55;
...
function fireGun() {
  ...
  network.send("fire", {
    ...
    yaw: local.yaw + recoilYaw + aimPunchYaw,
    pitch: local.pitch + recoilPitch + aimPunchPitch
  });
  updateWeaponHud();
  ...
  const direction = new THREE.Vector3();
  camera.getWorldDirection(direction);
  const end = origin.clone().addScaledVector(direction, 34);
  ...
  effects.spawnTracer(origin, end, info.color, heavy ? 1.35 : 0.72);
...
function setupNetworkHandlers() {
  ...
  network.on("init", (message) => {
    ...
    $("versionText").textContent = `v${message.version || "8.0.0"}`;
...
$("playButton").addEventListener("click", () => {
  ...
  firstPersonWeapon.setSkin(local.skin);
  network.send("hello", { name, roomCode: $("roomCodeInput").value, modePreference: $("modeSelect").value, skin: local.skin });
...
for (const id of ["pauseAdsMode", "pauseController", "pauseReducedMotion", "pauseColorblind", "crosshairSelect"]) $(id).addEventListener("change", updateAdvancedSetting);
...
$("skinSelect").addEventListener("change", (event) => { if (progression.setSkin(event.target.value)) { local.skin = event.target.value; firstPersonWeapon.setSkin(local.skin); network.send("hello", { name: local.name, roomCode: $("roomCodeInput").value, skin: local.skin }); } });
...
document.addEventListener("mousemove", (event) => {
  ...
  const info = WEAPON_INFO[local.weapon] || WEAPON_INFO.rifle;
  const zoom = zoomLevels[opticZoomIndex] || Math.max(1, settings.fov / Math.max(1, info.fov));
...
function updateCrosshair() {
  ...
  $("crosshair").style.opacity = local.blocking ? "0.45" : String(1 - adsAmount * 0.88);
}
...
function updateCameraFov(dt) {
  ...
  const scopedFov = THREE.MathUtils.clamp(settings.fov / zoomMultiplier, 16, info.fov || settings.fov);
  ...
  const showScope = adsAmount > 0.36 && settings.cameraMode === "first" && scopeType !== "none";
  scope.classList.toggle("hidden", !showScope);
  scope.classList.toggle("active", showScope);
  scope.style.setProperty("--ads", String(adsAmount));
