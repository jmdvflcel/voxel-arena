...
export class GamepadController {
  ...
  resetState() {
    ...
    state.jump = state.sprint = state.crouch = state.fire = state.aim = false;
    state.reloadPressed = state.grapplePressed = state.dashPressed = false;
    state.cameraPressed = state.nextWeaponPressed = state.previousWeaponPressed = false;
    return state;
  }

  poll() {
    if (!this.enabled || !navigator.getGamepads) return this.resetState();

    const pads = navigator.getGamepads();
    let pad = null;
    if (this.index != null) {
      for (let index = 0; index < pads.length; index++) {
        if (pads[index]?.index === this.index) { pad = pads[index]; break; }
      }
    }
    if (!pad) {
      for (let index = 0; index < pads.length; index++) {
    ...
    state.moveX = this.deadzone(pad.axes[0] || 0);
    state.moveY = this.deadzone(pad.axes[1] || 0);
    state.lookX = this.deadzone(pad.axes[2] || 0, 0.12);
    state.lookY = this.deadzone(pad.axes[3] || 0, 0.12);
    state.jump = Boolean(buttons[0]?.pressed);
    state.crouch = Boolean(buttons[1]?.pressed);
    state.reloadPressed = this.pressed(buttons, 2);
    state.cameraPressed = this.pressed(buttons, 3);
    state.grapplePressed = this.pressed(buttons, 4);
    state.dashPressed = this.pressed(buttons, 5);
    state.aim = Boolean(buttons[6]?.pressed);
    state.fire = Boolean(buttons[7]?.pressed);
    state.previousWeaponPressed = this.pressed(buttons, 14);
    state.nextWeaponPressed = this.pressed(buttons, 15);
    state.sprint = Boolean(buttons[10]?.pressed) || Math.hypot(pad.axes[0] || 0, pad.axes[1] || 0) > 0.92;

    this.previousButtons.length = buttons.length;
    for (let index = 0; index < buttons.length; index++) this.previousButtons[index] = Boolean(buttons[index]?.pressed);
    return state;
  }
}
