...
const PICKUP_CAMERA_POSITION = new THREE.Vector3();
const PICKUP_CAMERA_FORWARD = new THREE.Vector3();
const PICKUP_TO_LABEL = new THREE.Vector3();
...
export class PickupRenderer {
  ...
  update(time, camera = null, adsAmount = 0) {
    const cameraPosition = camera ? camera.getWorldPosition(PICKUP_CAMERA_POSITION) : null;
    const cameraForward = camera ? camera.getWorldDirection(PICKUP_CAMERA_FORWARD) : null;
    const ads = Math.max(0, Math.min(1, Number(adsAmount) || 0));
    for (const group of this.items.values()) {
      ...
      if (label?.material) {
        let opacity = 1 - ads * 0.68;
        if (cameraPosition && cameraForward && ads > 0.15) {
          PICKUP_TO_LABEL.copy(group.position).sub(cameraPosition);
          const distance = Math.max(0.001, PICKUP_TO_LABEL.length());
          const centerDot = cameraForward.dot(PICKUP_TO_LABEL.multiplyScalar(1 / distance));
          if (centerDot > 0.91) opacity *= THREE.MathUtils.lerp(1, 0.12, Math.min(1, (centerDot - 0.91) / 0.085));
          if (ads > 0.82) opacity *= 0.42;
        }
        label.material.opacity = Math.max(0.025, opacity);
      }
    }
  }
}
