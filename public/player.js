...
import { TEAM_COLORS, WEAPON_INFO, WEAPON_SKINS } from "./config.js";
...
export function createWeaponModel(name, firstPerson = false, skinName = "standard") {
  ...
  const accent = new THREE.MeshStandardMaterial({
    ...
    emissiveIntensity: (firstPerson ? 0.55 : 0.25) * skinProfile.emissiveScale,
    roughness: 0.22,
    metalness: 0.72
  });
  ...
  group.userData.skinMaterials = { dark, black, accent };
...
export class RemotePlayer {
  constructor(scene, player, quality) {
    ...
    this.scopeGlint.position.set(0, 1.58, -0.36);
  ...
  render(now, dt, interpolationDelay = this.interpolationDelay) {
    ...
    if (newer === older || seconds <= 0.001) {
      ...
      targetX = newer.x + newer.vx * extrapolation;
      targetY = newer.y + newer.vy * extrapolation;
      targetZ = newer.z + newer.vz * extrapolation;
    } else {
      targetX = hermiteScalar(older.x, newer.x, older.vx, newer.vx, alpha, seconds);
      targetY = hermiteScalar(older.y, newer.y, older.vy, newer.vy, alpha, seconds);
      targetZ = hermiteScalar(older.z, newer.z, older.vz, newer.vz, alpha, seconds);
    }

    const targetPosition = new THREE.Vector3(targetX, targetY, targetZ);
    const error = targetPosition.distanceTo(this.group.position);
    if (error > 4.5) this.group.position.copy(targetPosition);
    else this.group.position.lerp(targetPosition, Math.min(1, dt * (error > 1.2 ? 24 : 15)));
    ...
    const glintTarget = scoped ? 0.75 : 0;
    this.scopeGlint.material.opacity += (glintTarget - this.scopeGlint.material.opacity) * Math.min(1, dt * 12);
    if (scoped) {
...
export class FirstPersonWeapon {
  ...
  installAuthoredModel(name, source) {
    ...
    model.scale.copy(previous.scale);
    model.userData.authored = true;
    model.userData.muzzleLocal = name === "rifle"
  ...
  setSkin(name) {
    ...
    for (const [weaponName, oldModel] of this.models) {
      ...
      replacement.scale.copy(oldModel.scale);
  ...
  setObstruction(value) {
    this.obstruction = THREE.MathUtils.clamp(Number(value) || 0, 0, 1);
  }

  setBreath(value) {
    this.breath = THREE.MathUtils.clamp(Number(value) || 0, 0, 1);
  }
  ...
  melee(combo, type = "light") {
    this.swingCombo = Math.max(0, Math.min(2, Number(combo) || 0));
    this.meleeType = type;
    this.swingDuration = type === "heavy" ? 0.68 : type === "aerial" ? 0.48 : this.swingCombo === 2 ? 0.42 : 0.33;
    this.swing = 1;
  }
  ...
  update(dt, movementSpeed, grounded, aiming) {
    ...
    const targetRotation = this.targetRotation.set(
      ...
      isSword ? -0.22 : THREE.MathUtils.lerp(0.02, 0, this.aim) + this.obstruction * 0.58
    );
    ...
    if (this.swing > 0) {
      ...
      const progress = 1 - this.swing;
      const strike = Math.sin(Math.min(1, progress) * Math.PI);
      const windup = Math.min(1, progress / (this.meleeType === "heavy" ? 0.34 : 0.18));
      if (this.meleeType === "heavy") {
        targetRotation.x += -1.65 + progress * 2.75;
        targetRotation.y -= strike * 0.18;
        targetRotation.z -= strike * 0.42;
        targetPosition.y += (1 - windup) * 0.32 - strike * 0.36;
        targetPosition.z -= strike * 0.26;
      } else if (this.meleeType === "aerial") {
        targetRotation.x += -0.9 + progress * 2.05;
        targetRotation.z += 0.55 - progress * 1.1;
        targetPosition.y -= strike * 0.34;
