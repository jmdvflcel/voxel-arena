...
const ADS_VISIBILITY_PROFILES = Object.freeze({
  pistol:   { body: 0.22, arms: 0.12, housing: 0.30, glass: 0.05, reticle: 0.92 },
  smg:      { body: 0.10, arms: 0.055, housing: 0.16, glass: 0.04, reticle: 0.92 },
  rifle:    { body: 0.075, arms: 0.04, housing: 0.14, glass: 0.035, reticle: 0.94 },
  burst:    { body: 0.035, arms: 0.02, housing: 0.075, glass: 0.025, reticle: 0.92 },
  shotgun:  { body: 0.055, arms: 0.03, housing: 0.06, glass: 0.02, reticle: 0.96 },
  lmg:      { body: 0.025, arms: 0.015, housing: 0.05, glass: 0.02, reticle: 0.92 },
  marksman: { body: 0.00, arms: 0.00, housing: 0.00, glass: 0.00, reticle: 0.00 },
  railgun:  { body: 0.00, arms: 0.00, housing: 0.00, glass: 0.00, reticle: 0.00 }
});

const RETICLE_NAME_TOKENS = Object.freeze([
  "reticle", "centerdot", "holodot", "holoring", "frontpost",
  "frontorb", "frontbead", "fronthighlight", "chevron", "prismvertical",
  "prismhorizontal", "scope-dot"
]);
const GLASS_NAME_TOKENS = Object.freeze(["glass", "lens", "display"]);
const SIGHT_HOUSING_TOKENS = Object.freeze([
  "sight", "optic", "scope", "prism", "aperture", "notch", "eyepiece",
  "frame", "hood", "mount", "rib", "rearleft", "rearright", "rearbase", "ring"
]);

function viewModelMeshRole(object) {
  const key = String(object?.name || "").toLowerCase().replace(/[^a-z0-9]/g, "");
  if (RETICLE_NAME_TOKENS.some((token) => key.includes(token))) return "reticle";
  if (GLASS_NAME_TOKENS.some((token) => key.includes(token))) return "glass";
  if (SIGHT_HOUSING_TOKENS.some((token) => key.includes(token))) return "housing";
  return "body";
}

function prepareViewModelMesh(object, renderOrder = 1000) {
  if (!object?.isMesh) return;
  object.frustumCulled = false;
  object.castShadow = false;
  object.receiveShadow = false;
  const role = viewModelMeshRole(object);
  object.renderOrder = role === "reticle" ? renderOrder + 16 : role === "glass" ? renderOrder + 12 : role === "housing" ? renderOrder + 8 : renderOrder;
  if (!object.material) return;
  if (!object.userData.fpMaterialPrepared) {
    object.material = object.material.clone();
    object.userData.fpMaterialPrepared = true;
  }
  object.userData.fpRole = role;
  object.userData.fpBaseOpacity = Number.isFinite(object.material.opacity) ? object.material.opacity : 1;
  if (role === "glass") object.userData.fpBaseOpacity = Math.min(object.userData.fpBaseOpacity, 0.14);
  object.material.transparent = true;
  object.material.opacity = object.userData.fpBaseOpacity;
  object.material.depthTest = false;
  object.material.depthWrite = false;
  object.material.fog = false;
}

function cacheViewModelMeshes(group) {
  const roles = { body: [], housing: [], glass: [], reticle: [], arm: [] };
  group.traverse((object) => {
    if (!object.isMesh || !object.material) return;
    const role = object.userData.fpRole || viewModelMeshRole(object);
    (roles[role] || roles.body).push(object);
  });
  group.userData.fpMeshesByRole = roles;
  return roles;
}

function prepareViewModelGroup(group, renderOrder = 1000) {
  group.traverse((object) => prepareViewModelMesh(object, renderOrder));
  cacheViewModelMeshes(group);
  return group;
}
...
function addPistolIronSights(group, materials) {
  ...
  box(rear, [0.038, 0.092, 0.035], [-0.102, centerY - 0.043, rearZ], materials.secondary, "pistolRearLeft");
  box(rear, [0.038, 0.092, 0.035], [0.102, centerY - 0.043, rearZ], materials.secondary, "pistolRearRight");
  box(rear, [0.245, 0.025, 0.035], [0, centerY - 0.112, rearZ], materials.secondary, "pistolRearBase");
...
function addSmgApertureSight(group, materials) {
  ...
  torus(sight, 0.112, 0.010, [0, centerY, rearZ], materials.secondary, "smgRearRing", [0, 0, 0]);
  box(sight, [0.19, 0.030, 0.08], [0, centerY - 0.125, rearZ], materials.primary, "smgRearMount");
...
function addShotgunBeadSight(group, materials) {
  ...
  box(sight, [0.105, 0.032, 0.03], [-0.12, centerY - 0.055, rearZ], materials.secondary, "shotgunRearLeft");
  box(sight, [0.105, 0.032, 0.03], [0.12, centerY - 0.055, rearZ], materials.secondary, "shotgunRearRight");
  box(sight, [0.052, 0.018, 1.55], [0, centerY - 0.09, -0.73], materials.primary, "shotgunSightRib");
...
function addReflexSight(group, z, materials, variant = "rifle") {
  ...
  box(sight, [isLmg ? 0.22 : 0.18, 0.035, 0.19], [0, -0.12, 0], materials.secondary, "reflexMount");
  if (isLmg) {
    torus(sight, 0.165, 0.012, [0, 0.015, 0], frameColor, "lmgProtectiveHood", [0, 0, 0]);
    box(sight, [0.024, 0.235, 0.032], [-0.17, -0.015, 0], frameColor, "lmgReflexLeft");
    box(sight, [0.024, 0.235, 0.032], [0.17, -0.015, 0], frameColor, "lmgReflexRight");
  } else {
    box(sight, [0.024, 0.24, 0.03], [-0.125, 0.01, 0], frameColor, "holoFrameLeft");
    box(sight, [0.024, 0.24, 0.03], [0.125, 0.01, 0], frameColor, "holoFrameRight");
    box(sight, [0.30, 0.024, 0.03], [0, 0.145, 0], frameColor, "holoFrameTop");
  }
  box(sight, [isLmg ? 0.27 : 0.23, isLmg ? 0.23 : 0.21, 0.008], [0, 0.015, -0.018], materials.glass, "reflexGlass");
  if (isLmg) {
...
function addPrismSight(group, z, materials) {
  ...
  // Open-frame prism. The old solid housing occupied the target area.
  const halfWidth = 0.215;
  const halfHeight = 0.17;
  const frameDepth = 0.22;
  box(sight, [0.022, halfHeight * 2, frameDepth], [-halfWidth, 0, 0], materials.secondary, "prismFrameLeft");
  box(sight, [0.022, halfHeight * 2, frameDepth], [halfWidth, 0, 0], materials.secondary, "prismFrameRight");
  box(sight, [halfWidth * 2 + 0.022, 0.022, frameDepth], [0, halfHeight, 0], materials.secondary, "prismFrameTop");
  box(sight, [halfWidth * 2 + 0.022, 0.022, frameDepth], [0, -halfHeight, 0], materials.secondary, "prismFrameBottom");
  box(sight, [0.405, 0.315, 0.008], [0, 0, 0.19], materials.glass, "prismRearGlass");
  box(sight, [0.11, 0.025, 0.09], [0, -0.205, 0], materials.primary, "prismMount");
  box(sight, [0.004, 0.078, 0.006], [0, 0, 0.205], sightReticleMaterial(0xb6b0ff, 0.72), "prismVertical");
  box(sight, [0.078, 0.004, 0.006], [0, 0, 0.205], sightReticleMaterial(0xb6b0ff, 0.72), "prismHorizontal");
  sphere(sight, 0.012, [0, 0, 0.198], sightReticleMaterial(0xffd36f), "prismCenterDot");
  group.add(sight);
  registerSight(group, "burst-prism", [0, centerY, z + 0.205]);
  return sight;
}
...
export class RemotePlayer {
  ...
  render(now, dt, interpolationDelay = this.interpolationDelay, epochNow = Date.now()) {
    ...
    const powers = newer.powers || {};
    for (const key in powers) {
      if (key !== "dashReadyAt" && Number(powers[key]) > epochNow) { powerActive = true; break; }
    }
  ...
  muzzleWorldPosition(target = new THREE.Vector3()) {
    ...
    if (!this.weapon) return target.set(this.group.position.x, this.group.position.y + 1.45, this.group.position.z);
    target.copy(this.weapon.userData.muzzleLocal || TMP_POSITION.set(0, 0, -1.25));
    return this.weapon.localToWorld(target);
  }
...
const FP_VIEWMODEL_POSES = Object.freeze({
  pistol: {
    hipPosition: [0.30, -0.35, -0.64], hipRotation: [-0.045, -0.09, 0.018],
    adsDepth: -0.80, adsOffset: [0, 0], adsRotation: [0, 0, 0], armSpread: 0.03
  },
  smg: {
    hipPosition: [0.39, -0.41, -0.80], hipRotation: [-0.055, -0.13, 0.02],
    adsDepth: -1.02, adsOffset: [0, 0], adsRotation: [0, 0, 0], armSpread: 0.038
  },
  rifle: {
    hipPosition: [0.43, -0.43, -0.86], hipRotation: [-0.07, -0.12, 0.016],
    adsDepth: -1.14, adsOffset: [0, 0], adsRotation: [0, 0, 0], armSpread: 0.042
  },
  burst: {
    hipPosition: [0.42, -0.44, -0.88], hipRotation: [-0.06, -0.115, 0.01],
    adsDepth: -1.30, adsOffset: [0, 0], adsRotation: [0, 0, 0], armSpread: 0.042
  },
  shotgun: {
    hipPosition: [0.47, -0.47, -0.92], hipRotation: [-0.09, -0.14, 0.028],
    adsDepth: -1.22, adsOffset: [0, 0], adsRotation: [0, 0, 0], armSpread: 0.048
  },
  lmg: {
    hipPosition: [0.49, -0.51, -0.98], hipRotation: [-0.10, -0.15, 0.035],
    adsDepth: -1.42, adsOffset: [0, 0], adsRotation: [0, 0, 0], armSpread: 0.052
  },
  marksman: {
    hipPosition: [0.43, -0.46, -0.96], hipRotation: [-0.065, -0.115, 0.012],
    adsDepth: -1.46, adsOffset: [0, 0], adsRotation: [0, 0, 0], armSpread: 0.040
  },
  railgun: {
    hipPosition: [0.46, -0.51, -1.02], hipRotation: [-0.11, -0.16, 0.04],
    adsDepth: -1.56, adsOffset: [0, 0], adsRotation: [0, 0, 0], armSpread: 0.050
  }
});
...
function createFirstPersonArm(side, materials) {
  ...
  root.traverse((object) => {
    if (!object.isMesh) return;
    prepareViewModelMesh(object, 1001);
    object.userData.fpRole = "arm";
  });
  cacheViewModelMeshes(root);
  return root;
}
...
export class FirstPersonWeapon {
  constructor(camera) {
    ...
    this.viewModelRoot.matrixAutoUpdate = true;
    this.camera.add(this.viewModelRoot);
    // Preserve the original property for compatibility with the rest of the client.
    this.root = this.viewModelRoot;
    ...
    for (const name of Object.keys(WEAPON_INFO)) {
      ...
      prepareViewModelGroup(model, 1000);
      this.root.add(model);
  ...
  setVisible(visible) {
    ...
    this.viewModelRoot.visible = this.requestedVisible;
    if (this.requestedVisible) this.ensureActiveViewModel();
  }

  ensureActiveViewModel() {
    const active = this.models.get(this.current);
    for (const [name, model] of this.models) model.visible = name === this.current;
  ...
  installAuthoredModel(name, source) {
    ...
    prepareViewModelGroup(model, 1000);
    model.traverse((object) => {
      if (object.isMesh && object.material) object.material.toneMapped = true;
    });
  ...
  setSkin(name) {
    ...
    for (const [weaponName, oldModel] of this.models) {
      ...
      prepareViewModelGroup(replacement, 1000);
  ...
  updateArmPose(dt, isSword, reloadProgress, aiming, meleeProgress) {
    ...
    if (aiming && !isSword) {
      const pose = FP_VIEWMODEL_POSES[this.current] || FP_VIEWMODEL_POSES.rifle;
      const armSpread = Number(pose.armSpread || 0.04);
      this.armTargetRight.x += armSpread;
      this.armTargetLeft.x -= armSpread;
  ...
  update(dt, movementSpeed, grounded, aiming) {
    if (!this.requestedVisible) return;
    this.viewModelRoot.visible = true;
    const model = this.ensureActiveViewModel();
    if (!model) return;
    ...
    if (isSword) {
      ...
      this.hipPosition.fromArray(pose.hipPosition);
      const sight = model.userData.sightAnchor;
      const modelScale = Number(model.scale.x || presentation.scale || 1);
      const adsOffset = pose.adsOffset || [0, 0];
      if (sight?.isVector3) {
        this.aimPosition.set(
          -sight.x * modelScale + adsOffset[0],
          -sight.y * modelScale + adsOffset[1],
          Number(pose.adsDepth)
        );
      } else {
        this.aimPosition.set(adsOffset[0], adsOffset[1], Number(pose.adsDepth));
      }
    }
    ...
    const hipRotation = isSword ? [-0.08, -0.12, -0.22] : pose.hipRotation;
    const adsRotation = isSword ? hipRotation : pose.adsRotation;
    const targetRotation = this.targetRotation.set(
      THREE.MathUtils.lerp(hipRotation[0] - this.swayY * 1.8, adsRotation[0], this.aim) + this.recoilPosition * 0.08,
      THREE.MathUtils.lerp(hipRotation[1] - this.swayX * 1.8 + this.recoilYaw, adsRotation[1] + this.recoilYaw * 0.32, this.aim),
      THREE.MathUtils.lerp(hipRotation[2], adsRotation[2], this.aim) + this.obstruction * 0.58 + this.recoilRoll
    );
    ...
    // A true magnified optic becomes a clean lens once fully shouldered.
    // This prevents the scope body and hands from blocking the target.
    const scopedPrecisionWeapon = this.current === "marksman" || this.current === "railgun";
    const fullScopeBlend = scopedPrecisionWeapon
      ? smoothstep((this.aim - 0.20) / 0.56)
      : 0;
    this.updateAdsVisibility(model, dt, fullScopeBlend);
    const fullyHidden = scopedPrecisionWeapon && fullScopeBlend > 0.985;
    model.visible = !fullyHidden;
    if (fullyHidden) {
  ...
  updateAdsVisibility(model, dt, scopeBlend = 0) {
    const profile = ADS_VISIBILITY_PROFILES[this.current] || { body: 0.08, arms: 0.04, housing: 0.12, glass: 0.03, reticle: 0.94 };
    const aimBlend = smoothstep(this.aim);
    const precisionScope = this.current === "marksman" || this.current === "railgun";
    const opacityBlend = 1 - Math.exp(-dt * 20);
    const precisionFade = precisionScope ? Math.max(0, 1 - scopeBlend * 1.45) : 1;

    const roleTarget = (role) => {
      if (precisionScope) return precisionFade;
      const end = Number(profile[role] ?? profile.body ?? 0.08);
      return THREE.MathUtils.lerp(1, end, aimBlend);
    };

    const roleMeshes = model.userData.fpMeshesByRole || cacheViewModelMeshes(model);
    for (const role of ["body", "housing", "glass", "reticle"]) {
      const targetMultiplier = roleTarget(role);
      for (const object of roleMeshes[role]) {
        const base = Number(object.userData.fpBaseOpacity ?? 1);
        const target = base * targetMultiplier;
        object.material.opacity += (target - object.material.opacity) * opacityBlend;
        object.visible = role === "reticle" || object.material.opacity > 0.006;
      }
    }

    const armTarget = precisionScope
      ? precisionFade
      : THREE.MathUtils.lerp(1, Number(profile.arms ?? 0.04), aimBlend);
    for (const arm of [this.leftArm, this.rightArm]) {
      const armMeshes = arm.userData.fpMeshesByRole?.arm || cacheViewModelMeshes(arm).arm;
      for (const object of armMeshes) {
        const base = Number(object.userData.fpBaseOpacity ?? 1);
        const target = base * armTarget;
        object.material.opacity += (target - object.material.opacity) * opacityBlend;
        object.visible = object.material.opacity > 0.006;
      }
      arm.visible = armTarget > 0.006;
    }
  }

  muzzleWorldPosition(target = new THREE.Vector3()) {
    const model = this.models.get(this.current);
    const fallback = (this.current === "sword" || this.current === "voidblade")
      ? TMP_POSITION.set(0, 2.15, 0)
      : TMP_POSITION.set(0, 0, -1.2);
    target.copy(model.userData.muzzleLocal || fallback);
