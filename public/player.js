...
function addOptic(group, z, length, materials, magnified = false) {
  const optic = new THREE.Group();
  optic.name = "optic";
  optic.position.set(0, magnified ? 0.36 : 0.3, z);
  cylinder(optic, magnified ? 0.115 : 0.095, length, [0, 0, 0], materials.secondary, "opticBody");
  cylinder(optic, magnified ? 0.095 : 0.075, 0.025, [0, 0, -length * 0.52], materials.glass, "opticLens");
  box(optic, [0.05, 0.08, 0.17], [-0.13, 0.02, 0], materials.primary, "opticKnob");
  box(optic, [0.16, 0.04, 0.12], [0, -0.12, 0], materials.secondary, "opticMount");
  group.add(optic);
  return optic;
}

function addIronSights(group, frontZ, rearZ, height, materials, width = 0.18) {
  const sights = new THREE.Group();
  sights.name = "ironSights";
  box(sights, [0.035, 0.11, 0.045], [0, height, frontZ], materials.accent, "frontSightPost");
  box(sights, [width, 0.055, 0.045], [0, height - 0.012, rearZ], materials.secondary, "rearSightBase");
  box(sights, [0.04, 0.085, 0.045], [-width * 0.44, height + 0.035, rearZ], materials.secondary, "rearSightLeft");
  box(sights, [0.04, 0.085, 0.045], [width * 0.44, height + 0.035, rearZ], materials.secondary, "rearSightRight");
  group.add(sights);
  return sights;
}

function addReflexSight(group, z, materials) {
  const sight = new THREE.Group();
  sight.name = "reflexSight";
  sight.position.set(0, 0.34, z);
  box(sight, [0.2, 0.055, 0.22], [0, -0.08, 0], materials.secondary, "reflexMount");
  box(sight, [0.035, 0.21, 0.035], [-0.095, 0.04, 0], materials.primary, "reflexFrameLeft");
  box(sight, [0.035, 0.21, 0.035], [0.095, 0.04, 0], materials.primary, "reflexFrameRight");
  box(sight, [0.22, 0.035, 0.035], [0, 0.145, 0], materials.primary, "reflexFrameTop");
  box(sight, [0.17, 0.15, 0.012], [0, 0.04, -0.018], materials.glass, "reflexGlass");
  sphere(sight, 0.016, [0, 0.04, -0.04], materials.accent, "reflexDot");
  group.add(sight);
  return sight;
}
...
function buildPistol(group, m) {
  ...
  box(group, [0.16, 0.08, 0.04], [0, 0.32, -0.56], m.accent, "frontSight");
  box(group, [0.18, 0.07, 0.04], [0, 0.31, 0.08], m.secondary, "rearSight");
...
function buildRifle(group, m, burst = false) {
  ...
  if (burst) addOptic(group, -0.26, 0.48, m, true);
  else addReflexSight(group, -0.26, m);
...
function buildShotgun(group, m) {
  ...
  addIronSights(group, -1.62, 0.12, 0.27, m, 0.22);
...
function buildMarksman(group, m) {
  ...
  addOptic(group, -0.3, 0.82, m, true);
...
function buildRailgun(group, m) {
  ...
  addOptic(group, -0.25, 0.88, m, true);
...
function buildFallbackGun(group, name, m) {
  ...
  if (name === "smg") addIronSights(group, -length * 0.88, 0.18, height * 0.72, m, 0.19);
  else if (name === "lmg") addReflexSight(group, -0.2, m);
  else addOptic(group, -0.2, 0.32, m, false);
...
const FP_AIM_LAYOUTS = Object.freeze({
  pistol: [0, -0.315, -0.53], smg: [0, -0.315, -0.59], rifle: [0, -0.365, -0.62],
  burst: [0, -0.365, -0.64], shotgun: [0, -0.275, -0.64], lmg: [0, -0.36, -0.68],
  marksman: [0, -0.365, -0.72], railgun: [0, -0.37, -0.75]
});
...
export class FirstPersonWeapon {
  ...
  installAuthoredModel(name, source) {
    ...
    if (name === "shotgun" || name === "rifle") {
      model.traverse((object) => {
        const key = String(object.name || "").toLowerCase();
        if (key.includes("optic")) object.visible = false;
      });
      const materials = weaponMaterials(name, true, this.skinName);
      if (name === "shotgun") addIronSights(model, -1.62, 0.12, 0.27, materials, 0.22);
      else addReflexSight(model, -0.26, materials);
    }
    model.traverse((object) => {
      if (!object.isMesh) return;
      object.frustumCulled = false;
    ...
    for (const [weaponName, oldModel] of this.models) {
      if (oldModel.userData.authored) {
        ...
        oldModel.traverse((object) => {
          ...
          const key = String(object.name || "").toLowerCase();
          if (key.includes("blade") || key.includes("energy") || key.includes("accent") || key.includes("light")) return;
  ...
  updateArmPose(dt, isSword, reloadProgress, aiming, meleeProgress) {
    ...
    if (aiming && !isSword) {
      this.armTargetRight.x -= 0.08;
      this.armTargetLeft.x += 0.08;
      this.armTargetRight.y += 0.03;
      this.armTargetLeft.y += 0.03;
      rightRotX = -0.32;
      leftRotX = -0.4;
    }
  ...
  update(dt, movementSpeed, grounded, aiming) {
    ...
    const ads = FP_AIM_LAYOUTS[this.current] || [0, -0.31, -0.62];
    if (isSword) {
    ...
    const targetRotation = this.targetRotation.set(
      -0.08 - this.swayY * 1.8 + this.recoilPosition * 0.08,
