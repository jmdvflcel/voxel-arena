...
const ADS_VISIBILITY_PROFILES = Object.freeze({
  pistol:   { body: 0.76, arms: 0.42 },
  smg:      { body: 0.52, arms: 0.24 },
  rifle:    { body: 0.44, arms: 0.20 },
  burst:    { body: 0.30, arms: 0.13 },
  shotgun:  { body: 0.54, arms: 0.24 },
  lmg:      { body: 0.24, arms: 0.10 },
  marksman: { body: 0.00, arms: 0.00 },
  railgun:  { body: 0.00, arms: 0.00 }
});

const SIGHT_NAME_TOKENS = Object.freeze([
  "sight", "reticle", "glass", "lens", "optic", "scope", "holo",
  "prism", "bead", "post", "ring", "chevron", "aperture", "notch",
  "eyepiece", "display"
]);

function viewModelMeshRole(object) {
  const key = String(object?.name || "").toLowerCase();
  return SIGHT_NAME_TOKENS.some((token) => key.includes(token)) ? "sight" : "body";
}
...
function prepareViewModelMesh(object, renderOrder = 1000) {
  ...
  object.renderOrder = viewModelMeshRole(object) === "sight" ? renderOrder + 12 : renderOrder;
  if (!object.material) return;
  if (!object.userData.fpMaterialPrepared) {
    object.material = object.material.clone();
    object.userData.fpMaterialPrepared = true;
  }
  object.userData.fpRole = viewModelMeshRole(object);
  object.userData.fpBaseOpacity = Number.isFinite(object.material.opacity) ? object.material.opacity : 1;
  const key = String(object.name || "").toLowerCase();
  if (object.userData.fpRole === "sight" && (key.includes("glass") || key.includes("lens") || key.includes("display"))) {
    object.userData.fpBaseOpacity = Math.min(object.userData.fpBaseOpacity, 0.22);
  }
...
export class RemotePlayer {
  ...
  updateAdsVisibility(model, dt, scopeBlend = 0) {
    const profile = ADS_VISIBILITY_PROFILES[this.current] || { body: 0.5, arms: 0.25 };
    const aimBlend = smoothstep(this.aim);
    const precisionScope = this.current === "marksman" || this.current === "railgun";
    const bodyTarget = precisionScope
      ? Math.max(0, 1 - scopeBlend * 1.35)
      : THREE.MathUtils.lerp(1, profile.body, aimBlend);
    const armTarget = precisionScope
      ? Math.max(0, 1 - scopeBlend * 1.55)
      : THREE.MathUtils.lerp(1, profile.arms, aimBlend);
    const opacityBlend = 1 - Math.exp(-dt * 18);

    model.traverse((object) => {
      if (!object.isMesh || !object.material) return;
      const base = Number(object.userData.fpBaseOpacity ?? 1);
      const role = object.userData.fpRole || viewModelMeshRole(object);
      const target = base * (role === "sight" ? 1 : bodyTarget);
      object.material.opacity += (target - object.material.opacity) * opacityBlend;
      object.visible = role === "sight" || object.material.opacity > 0.012;
    });
    ...
    for (const arm of [this.leftArm, this.rightArm]) {
      arm.traverse((object) => {
        ...
        object.material.opacity += (target - object.material.opacity) * opacityBlend;
        object.visible = object.material.opacity > 0.012;
      });
      arm.visible = armTarget > 0.012;
    }
  }
...
const FP_VIEWMODEL_POSES = Object.freeze({
  pistol: {
    ...
    adsDepth: -0.72, adsOffset: [0, 0.003], adsRotation: [0, 0, 0], armSpread: 0.03
  },
  smg: {
    hipPosition: [0.39, -0.41, -0.80], hipRotation: [-0.055, -0.13, 0.02],
    adsDepth: -0.90, adsOffset: [0, -0.004], adsRotation: [0, 0, 0], armSpread: 0.038
  },
  rifle: {
    hipPosition: [0.43, -0.43, -0.86], hipRotation: [-0.07, -0.12, 0.016],
    adsDepth: -1.02, adsOffset: [0, -0.008], adsRotation: [0, 0, 0], armSpread: 0.042
  },
  burst: {
    hipPosition: [0.42, -0.44, -0.88], hipRotation: [-0.06, -0.115, 0.01],
    adsDepth: -1.16, adsOffset: [0, -0.016], adsRotation: [0, 0, 0], armSpread: 0.042
  },
  shotgun: {
    hipPosition: [0.47, -0.47, -0.92], hipRotation: [-0.09, -0.14, 0.028],
    adsDepth: -1.06, adsOffset: [0, -0.004], adsRotation: [0, 0, 0], armSpread: 0.048
  },
  lmg: {
    hipPosition: [0.49, -0.51, -0.98], hipRotation: [-0.10, -0.15, 0.035],
    adsDepth: -1.24, adsOffset: [0, -0.018], adsRotation: [0, 0, 0], armSpread: 0.052
  },
  marksman: {
    hipPosition: [0.43, -0.46, -0.96], hipRotation: [-0.065, -0.115, 0.012],
    adsDepth: -1.42, adsOffset: [0, -0.024], adsRotation: [0, 0, 0], armSpread: 0.040
  },
  railgun: {
    hipPosition: [0.46, -0.51, -1.02], hipRotation: [-0.11, -0.16, 0.04],
    adsDepth: -1.52, adsOffset: [0, -0.028], adsRotation: [0, 0, 0], armSpread: 0.050
  }
});
