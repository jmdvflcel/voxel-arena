...
for (const name of names) {
  ...
  if (!name.includes("sword") && name !== "voidblade") {
    ...
    const fullScopeWeapon = name === "marksman" || name === "railgun";
    if (!fullScopeWeapon && !activeModel.visible) throw new Error("ADS hid the weapon model for " + name);
    if (fullScopeWeapon && activeModel.visible) throw new Error("Full-scope viewmodel still blocks the lens for " + name);
...
actor.dispose();
console.log("Presentation runtime test passed: camera-mounted ViewModelRoot, clear full-scope optics, visible hip-fire weapons and arms, eight unique centered sights, recoil, reloads, and melee.");
`);
