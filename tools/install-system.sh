...
grep -q '^MAX_CLIENTS=' /etc/voxel-arena.env || echo 'MAX_CLIENTS=32' >> /etc/voxel-arena.env
# Remove any legacy bot setting from older releases. Remove any legacy setting from older releases.
sed -i '/^BOT_COUNT=/d' /etc/voxel-arena.env
chmod 640 /etc/voxel-arena.env

cat > /etc/systemd/system/voxel-arena.service <<SERVICE
[Unit]
Description=Voxel Combat Arena v9.1 Multiplayer Server
