...
if (!main.includes('origin: { x: aimOrigin.x, y: aimOrigin.y, z: aimOrigin.z }')) throw new Error("Client does not send its validated camera origin");
...
if (!serverSource.includes('roster: fullRoster()')) throw new Error("Snapshots do not include an authoritative roster");
...
if (packageJson.version !== "8.9.0") throw new Error(`Unexpected package version ${packageJson.version}`);
if (!serverSource.includes('const GAME_VERSION = "8.9.0"')) throw new Error("Server version is not synchronized");
