...
for (const [weapon, sight] of [["pistol", "iron"], ["smg", "iron"], ["shotgun", "iron"], ["rifle", "reflex"], ["lmg", "reflex"], ["marksman", "sniper"], ["railgun", "rail"]]) {
  const line = configSource.split("\n").find((candidate) => candidate.trimStart().startsWith(`${weapon}: {`));
  if (!line || !line.includes(`scope: "${sight}"`)) throw new Error(`Unexpected sight mode for ${weapon}`);
}
...
if (packageJson.version !== "8.2.0") throw new Error(`Unexpected package version ${packageJson.version}`);
if (!serverSource.includes('const GAME_VERSION = "8.2.0"')) throw new Error("Server version is not synchronized");
