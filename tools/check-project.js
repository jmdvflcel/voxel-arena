...
const nodeFiles = ["server.js", "tools/check-project.js", "tools/presentation-test.js", "tools/smoke-test.js", "tools/load-test.js"];
const requiredFiles = [
  "package.json", "server.js", "README.md", "CHANGELOG.md", ".gitignore", ".github/workflows/ci.yml",
  "user-data.sh", "update-existing-instance.sh", "tools/install-system.sh", "tools/presentation-test.js", "tools/load-test.js",
...
if (!main.includes('direction: { x: shotAimDirection.x, y: shotAimDirection.y, z: shotAimDirection.z }')) throw new Error("Client does not send the camera-center aim direction");
...
if (!serverSource.includes('const precisionOptic = weapon === WEAPONS.marksman || weapon === WEAPONS.railgun;')) throw new Error("Precision optic spread tuning is missing");
if (!serverSource.includes('player.weaponBloom * (aiming ? (precisionOptic ? 0.05 : 0.28) : 1)')) throw new Error("ADS bloom is not sight-truth tuned");
if (!serverSource.includes('if (fullyAimed && Number(weapon.pellets || 1) === 1) return 0;')) throw new Error("Full ADS shots are not exactly sight-true");
if (!main.includes('origin: { x: shotAimOrigin.x, y: shotAimOrigin.y, z: shotAimOrigin.z }')) throw new Error("Client does not send its validated camera origin");
if (!serverSource.includes('horizontalError <= 0.28')) throw new Error("Server camera-origin validation is missing");
if (!playerSource.includes('ADS_VISIBILITY_PROFILES')) throw new Error("Per-weapon ADS visibility profiles are missing");
if (!playerSource.includes('updateAdsVisibility(model, dt, fullScopeBlend)')) throw new Error("ADS body and arm visibility fading is missing");
if (playerSource.includes('"prismHousing"')) throw new Error("Trident prism still contains a solid obstructive housing");

if (!main.includes('const elementCache = new Map()')) throw new Error("DOM element caching is missing");
if (!main.includes('nextRadarAt = now + 66')) throw new Error("Radar updates are not frame-rate throttled");
if (!main.includes('nextDayNightAt = now + 100')) throw new Error("Day/night updates are not throttled");
if (!playerSource.includes('fpMeshesByRole')) throw new Error("Viewmodel mesh-role caching is missing");
if (!serverSource.includes('MAX_SNAPSHOT_BUFFER')) throw new Error("Snapshot backpressure protection is missing");
if (!serverSource.includes('maxPayload: 64 * 1024')) throw new Error("WebSocket payload limiting is missing");
if (!serverSource.includes('eventLoopLagMs')) throw new Error("Event-loop health telemetry is missing");
if (!serverSource.includes('function shutdown(signal)')) throw new Error("Graceful shutdown handling is missing");

if (!serverSource.includes('roster,\n    round: roundSnapshot')) throw new Error("Snapshots do not include the cached authoritative roster");
if (!serverSource.includes('function publicRosterPlayer(player)')) throw new Error("Lightweight roster serialization is missing");
if (!main.includes('const rosterData = new Map()')) throw new Error("Client authoritative roster cache is missing");
if (!main.includes('resetNetworkPlayerState();')) throw new Error("Reconnect state is not reset on init");
if (!main.includes('syncRoster(message.roster || message.players || [])')) throw new Error("Snapshots do not synchronize the authoritative roster");
const networkSource = fs.readFileSync(path.join(root, "public/network.js"), "utf8");
if (!networkSource.includes('this.connectionGeneration')) throw new Error("WebSocket generation guard is missing");
if (!networkSource.includes('if (!isCurrent()) return')) throw new Error("Stale WebSocket events are not ignored");
if (serverSource.includes('function createBot(') || serverSource.includes('function updateBots(') || serverSource.includes('BOT_COUNT')) throw new Error("Bot server code is still present");
const installSource = fs.readFileSync(path.join(root, "tools/install-system.sh"), "utf8");
if (!installSource.includes("sed -i '/^BOT_COUNT=/d' /etc/voxel-arena.env")) throw new Error("Legacy bot configuration is not removed during install");

const packageJson = JSON.parse(fs.readFileSync(path.join(root, "package.json"), "utf8"));
if (packageJson.version !== "9.1.0") throw new Error(`Unexpected package version ${packageJson.version}`);
if (!serverSource.includes('const GAME_VERSION = "9.1.0"')) throw new Error("Server version is not synchronized");
