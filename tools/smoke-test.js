...
(async () => {
  ...
  try {
    ...
    if (status.version !== "9.1.0") throw new Error(`Unexpected version: ${status.version}`);
    if (status.tickRate !== 30) throw new Error(`Unexpected tick rate: ${status.tickRate}`);

    const page = await fetch(`${base}/`);
    const html = await page.text();
    if (!page.ok || !html.includes("Voxel Combat Arena") || !html.includes("v9.1")) {
    ...
    if (version.version !== "9.1.0") throw new Error("Version endpoint failed");
