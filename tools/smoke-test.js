...
function connectClient(index) {
  return new Promise((resolve, reject) => {
    ...
    socket.on("message", (data) => {
      ...
      clearTimeout(timer);
      socket.send(JSON.stringify({ type: "hello", name: `Smoke-${index}` }));
...
(async () => {
  ...
  try {
    ...
    if (status.version !== "7.0.0") throw new Error(`Unexpected version: ${status.version}`);
    if (status.tickRate !== 30) throw new Error(`Unexpected tick rate: ${status.tickRate}`);

    const page = await fetch(`${base}/`);
    const html = await page.text();
    if (!page.ok || !html.includes("Voxel Combat Arena") || !html.includes("v7")) {
    ...
    actor.socket.send(JSON.stringify({ type: "melee", weapon: "sword", combo: 0, latency: 24 }));
