...
function connectClient(index) {
  return new Promise((resolve, reject) => {
    ...
    socket.on("message", (data) => {
      ...
      clearTimeout(timer);
      socket.send(JSON.stringify({ type: "hello", name: `Smoke-${index}`, skin: "standard", modePreference: "tdm" }));
...
(async () => {
  ...
  try {
    ...
    if (status.version !== "8.0.0") throw new Error(`Unexpected version: ${status.version}`);
    ...
    if (version.version !== "8.0.0") throw new Error("Version endpoint failed");
