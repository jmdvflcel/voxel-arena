...
function setupNetworkHandlers() {
  ...
  network.on("init", (message) => {
    ...
    $("versionText").textContent = `v${message.version || "8.3.0"}`;
...
function updateCameraFov(dt) {
  ...
  const showScope = adsAmount > 0.84 && settings.cameraMode === "first" && SCOPE_OVERLAY_TYPES.has(scopeType);
  scope.classList.toggle("hidden", !showScope);
  scope.classList.toggle("active", showScope);
  scope.style.setProperty("--ads", String(showScope ? THREE.MathUtils.smoothstep(adsAmount, 0.8, 1) : 0));
