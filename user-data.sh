...
if command -v flock >/dev/null 2>&1; then
  exec 9>/var/lock/voxel-arena-install.lock
  flock -n 9 || {
    echo "Another Voxel Arena installation is already running"
    exit 1
  }
fi
...
for attempt in 1 2 3; do
  set +e
  timeout 180 runuser -u "$APP_USER" -- \
    env npm_config_cache=/tmp/voxel-npm-cache \
    npm install \
      --omit=dev \
      --no-audit \
      --no-fund \
  ...
  if [ -f node_modules/express/package.json ] \
    ...
    && [ -f node_modules/three/build/three.module.js ]; then
    echo "Dependencies verified (npm exit $npm_exit)"
    break
  fi

  echo "Dependency attempt $attempt failed"
...
[ -f node_modules/express/package.json ] || {
  echo "Dependencies unavailable"
  exit 1
}

timeout 120 runuser -u "$APP_USER" -- npm run check
timeout 120 runuser -u "$APP_USER" -- npm run presentation
timeout 120 runuser -u "$APP_USER" -- env npm run smoke
...
APP_DIR="$APP_DIR" \
APP_USER="$APP_USER" \
bash "$APP_DIR/tools/install-system.sh"
...
for attempt in $(seq 1 35); do
  if curl -fsS \
      ...
      > /tmp/voxel-status.json \
    && grep -q '"version":"9.1.0"' /tmp/voxel-status.json; then

    cat /tmp/voxel-status.json
    echo
    echo "Health check passed"

    rm -rf "$BACKUP_DIR" /tmp/voxel-npm-cache

    echo "=== Voxel Combat Arena v9.1 installation complete ==="
    exit 0
  fi
