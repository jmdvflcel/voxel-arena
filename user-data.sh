...
echo "=== Voxel Combat Arena v8.4 resilient installation ==="
...
runuser -u "$APP_USER" -- env BOT_COUNT=0 npm run smoke
...
for attempt in $(seq 1 35); do
  if curl -fsS http://127.0.0.1:3000/api/status >/tmp/voxel-status.json; then cat /tmp/voxel-status.json; echo; echo "Health check passed"; rm -rf "$BACKUP_DIR"; echo "=== Voxel Combat Arena v8.4 installation complete ==="; exit 0; fi
  sleep 2
done
