...
if command -v flock >/dev/null 2>&1; then
  ...
  flock -n 9 || { echo "Another Voxel Arena installation is already running"; exit 1; }
fi
...
for attempt in 1 2 3; do
  set +e
  timeout 180 runuser -u "$APP_USER" -- env npm_config_cache=/tmp/voxel-npm-cache npm install --omit=dev --no-audit --no-fund --package-lock=false
  npm_exit=$?
  set -e
...
for attempt in $(seq 1 35); do
  if curl -fsS http://127.0.0.1:3000/api/status >/tmp/voxel-status.json && grep -q '"version":"9.1.0"' /tmp/voxel-status.json; then cat /tmp/voxel-status.json; echo; echo "Health check passed"; rm -rf "$BACKUP_DIR" /tmp/voxel-npm-cache; echo "=== Voxel Combat Arena v9.1 installation complete ==="; exit 0; fi
  sleep 2
done
