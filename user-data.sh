...
echo "=== Voxel Combat Arena v8 resilient installation ==="
...
for attempt in $(seq 1 35); do
  if curl -fsS http://127.0.0.1:3000/api/status >/tmp/voxel-status.json; then cat /tmp/voxel-status.json; echo; echo "Health check passed"; rm -rf "$BACKUP_DIR"; echo "=== Voxel Combat Arena v8 installation complete ==="; exit 0; fi
  sleep 2
done
