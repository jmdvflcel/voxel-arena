...
echo "=== Voxel Combat Arena v7 deployment starting ==="
...
# Use the committed lockfile when available for deterministic production builds.
rm -rf node_modules
...
for attempt in 1 2 3; do
  ...
  if runuser -u "$APP_USER" -- env npm_config_cache=/tmp/voxel-npm-cache "${INSTALL_COMMAND[@]}"; then
    NPM_OK=1
    break
  fi
...
[Unit]
Description=Voxel Combat Arena v7 Multiplayer Server
...
echo "=== Voxel Combat Arena v7 deployment complete ==="
